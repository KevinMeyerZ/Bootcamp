    ...
    $params = $request->getParams();
        ...
        if (isset($params['email']) && !empty($params['email'])) {
            ...
            for ($x = 0; $x < count($model); $x++) {
                if($model[$x]->email == $params['email'] && $model[$x]->password == sha1($params['password'])){
           ...
           if(!empty($data_akhirku)){
            $_SESSION['user']['nama'] = $data_akhirku->nama;
            $_SESSION['user']['id'] = $data_akhirku->id;
...
$app->post('/auth/transaksi/{id}',function ($request,$response){
   ...
   $db = Db::db();
    ...
    $data = $db->select('*')
    ->from('t_transaksi')
    ->where('t_transaksi.kode','=',$id);

    $model = $data->findAll();

    for($i=0; $i<count($model); $i++){
       $db->select('t_transaksi.kode,m_produk.nama,t_transaksi_det.qty,m_produk.harga')
    ->from('t_transaksi')
    ->leftJoin('t_transaksi_det','t_transaksi.kode = t_transaksi_det.kode')
    ->leftJoin('m_produk','t_transaksi_det.id_produk = m_produk.id_produk')
    ->where('t_transaksi.kode','=',$model[$i]->kode);
...
$app->post('/auth/qty',function ($request,$response){
    $db = Db::db();
    ...
    for($x=0; $x<count($id); $x++){
        $update = $db->update('t_transaksi_det',['qty' => $qty[$x]],['id_detail' => $id[$x]]);
        $sukses = 'Sukses Brow!';
        $h_id = 'Data Berhasil di Update!!';
    }
