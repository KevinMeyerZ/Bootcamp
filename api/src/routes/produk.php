...
$app->get('/produk/data_produk',function ($request,$response){
    $db = Db::db();
...
$app->post('/produk/hapus_data',function ($request,$response){
    $db = Db::db();
...
$app->post('/produk/tambah_produk',function ($request,$response){
    $params = $request->getParams();
    $db = Db::db();
    ...
    for($x=0; $x<count($toping); $x++){
        if($x == 0){
            $data_topping = $toping[$x];
        }else{
            $dataku = $dataku.",".$toping[$x];
        }
    }

    $final = $data_topping.$dataku;
...
$app->post('/produk/update_produk',function ($request,$response){
    $db = Db::db();
    $input = $request->getParsedBody();
    ...
    if($input['ufileSource'] == null){
        $outputfoto = 'Foto tetap';
    }else{
        $folderPath = "assets/img/upload_produk/";
        $image_parts = explode(";base64,", $input['ufileSource']);
        $image_type_aux = explode("image/", $image_parts[0]);
        $image_type = $image_type_aux[1];
        $image_base64 = base64_decode($image_parts[1]);
        $file = $folderPath . uniqid() . '.png';
        compress($input['ufileSource'], $file, 50);
    }
    ...
    if($data_update == null){
        ...
        if(isset($input['ufileSource'])&& !empty($input['ufileSource'])){
            $update = $db->update('m_produk',['nama' => $nama, 'harga' => $harga, 'id_kategori' => $id_kategori, 'deskripsi' => $deskripsi,'toping' => $final,'foto' => $file, 'hpp' => $hpp],['id_produk' => $data_update->id_produk]);
            $h_id = 'Data Berhasil di Update!!';
        }else{
            $update = $db->update('m_produk',['nama' => $nama, 'harga' => $harga, 'id_kategori' => $id_kategori, 'deskripsi' => $deskripsi,'toping' => $final,'hpp' => $hpp],['id_produk' => $data_update->id_produk]);
            $h_id = 'Data Berhasil di Update!!';
        } 
    }
